...
#include "nuevo_protocolo/myserial.hpp"
...
int PackSendPayload(uint8_t* payload, int lenPay, int num) {
	...
	//unsigned char *t="holasoymarianaperonomegusta";
	//auto mensaje = serial.read();
	serial.Close();
	std::cout<<messageSend<<" - "<<respuesta<< "\n";
...
int  main(void)
{
 ...
 /*    
   //mySerial serial("/dev/ttyACM0",115200);
	  //  serial.Send(messageSend, count);
	    
	    
	    
	    for(float i = 0; i < 1.00; i+=0.05){
    		VescUartSetDuty(0.2);	    
