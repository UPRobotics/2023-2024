          ...
          std::shared_ptr<std_srvs::srv::SetBool::Response> response)
{
    if(request->data==false){       
        ...
        modo_de_lectura=1;
        response->message="movimiento del brazo hola";
    }
    was_cero=0;
...
rclcpp::Client<formato::srv::Moverob>::SharedPtr clientRob;
rclcpp::Client<formato::srv::Movearm>::SharedPtr clientArm;
...
void MoveRob(std::vector<double> &axis, std::vector<bool> &botons){
    ...
    else if(axis[2]<=1.0 && axis[2]>0.3){
        flipper = 1;
    }// mover flipper delantero hacia arriba
    else if(axis[2]>=-1.0 && axis[2]<-0.3){
        flipper = 2;
    }// mover flipper delantero hacia abajo
    else if(axis[3]<=1.0 && axis[3]>0.3){
        flipper = 3;
    }// mover flipper trasero hacia arriba
    else if(axis[3]>=-1.0 && axis[3]<-0.3){
        flipper = 4;
    }// mover flipper trasero hacia abajo
    if(r==0 && flipper==0){
        if(was_cero) return;
	    was_cero=true;
    }else{
...
void MoveArm(std::vector<double> &axis, std::vector<bool> &botons){
    ...
    auto request = std::make_shared<formato::srv::Movearm::Request>();
...
int main(int argc, char ** argv){
	...
	clientRob = node->create_client<formato::srv::Moverob>("move_robot");	
	//clientArm = node->create_client<formato::srv::Movearm>("direcciones");	
